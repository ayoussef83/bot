...
export class ChannelAccountsService {
  ...
  private async verifyMetaTokenForId(
    ...
    try {
      // --- Instagram Business: validation is tricky because some tokens can't directly GET /{igId}.
      // We validate by using /me (page token) or /me/accounts (user token) to find the connected instagram_business_account.
      ...
      // --- Facebook Page / WhatsApp: direct lookup is fine
